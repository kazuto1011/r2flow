...
class PostProcess(nn.Module):
    def __init__(self, dim=4096, num_sectors=16):
        super().__init__()
        self.dim = dim
        self.num_sectors = num_sectors

    def forward(self, feats, agg_type: str = "subsample"):
        assert feats.ndim == 4, feats.shape
        if agg_type == "subsample" or agg_type == "lidargen":
            feats = einops.rearrange(feats, "B C H W -> B (C H W)")
            random.seed(0)
            feats = feats[:, random.sample(range(feats.shape[1]), self.dim)]
            return feats
        elif agg_type == "global":
            return feats.mean(dim=[2, 3])
        elif agg_type == "angle":
            feats = einops.rearrange(
                feats, "B C H (N W) -> B (C N) (H W)", N=self.num_sectors
            )
            return feats.mean(dim=-1)
        elif agg_type == "depth":
            feats = einops.rearrange(
                feats, "B C (N H) W -> B (C N) (H W)", N=self.num_sectors
            )
            return feats.mean(dim=-1)
