...
from .torchsparse_utils import basic_blocks
from .torchsparse_utils.utils import (
    initial_voxelize,
    point_to_voxel,
    voxel_to_point,
)
...
class SPVCNN(nn.Module):
    def __init__(self, config):
        ...
        self.stage1 = nn.Sequential(
            basic_blocks.BasicConvolutionBlock(
                cs[0], cs[0], ks=2, stride=2, dilation=1
            ),
        ...
        self.stage2 = nn.Sequential(
            basic_blocks.BasicConvolutionBlock(
                cs[1], cs[1], ks=2, stride=2, dilation=1
            ),
        ...
        self.stage3 = nn.Sequential(
            basic_blocks.BasicConvolutionBlock(
                cs[2], cs[2], ks=2, stride=2, dilation=1
            ),
        ...
        self.stage4 = nn.Sequential(
            basic_blocks.BasicConvolutionBlock(
                cs[3], cs[3], ks=2, stride=2, dilation=1
            ),
        ...
        self.up1 = nn.ModuleList(
            [
                ...
                nn.Sequential(
                    basic_blocks.ResidualBlock(
                        cs[5] + cs[3], cs[5], ks=3, stride=1, dilation=1
                    ),
                    basic_blocks.ResidualBlock(
                        cs[5], cs[5], ks=3, stride=1, dilation=1
                    ),
                ),
            ]
        )
        ...
        self.up2 = nn.ModuleList(
            [
                ...
                nn.Sequential(
                    basic_blocks.ResidualBlock(
                        cs[6] + cs[2], cs[6], ks=3, stride=1, dilation=1
                    ),
                    basic_blocks.ResidualBlock(
                        cs[6], cs[6], ks=3, stride=1, dilation=1
                    ),
                ),
            ]
        )
        ...
        self.up3 = nn.ModuleList(
            [
                ...
                nn.Sequential(
                    basic_blocks.ResidualBlock(
                        cs[7] + cs[1], cs[7], ks=3, stride=1, dilation=1
                    ),
                    basic_blocks.ResidualBlock(
                        cs[7], cs[7], ks=3, stride=1, dilation=1
                    ),
                ),
            ]
        )
        ...
        self.up4 = nn.ModuleList(
            [
                ...
                nn.Sequential(
                    basic_blocks.ResidualBlock(
                        cs[8] + cs[0], cs[8], ks=3, stride=1, dilation=1
                    ),
                    basic_blocks.ResidualBlock(
                        cs[8], cs[8], ks=3, stride=1, dilation=1
                    ),
                ),
            ]
        )
    ...
    def forward(self, data_dict, feature="penultimate"):
        x = data_dict["lidar"]
...
def pretrained_model(
    url_or_file: str, progress: bool = True, compile: bool = False, **kwargs
):
    from .utls import setup_voxel_models_from_lidm

    return setup_voxel_models_from_lidm(
        url_or_file, SPVCNN, progress, compile, **kwargs
    )
